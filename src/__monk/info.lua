...
local function _get_monk_cli_result(exitcode, stdout, stderr)
    if exitcode ~= 0 then
...
if _info.monkd == 'running' then
    ...
    if am.app.get_configuration("NODE_PRIVKEY") or am.app.get_configuration({"DAEMON_CONFIGURATION", "masternode"})  then
        ...
        local _exitcode, _stdout, _stderr = _exec_monk_cli("-datadir=data", "getmasternodestatus")
        local _success, _output = _get_monk_cli_result(_exitcode, _stdout, _stderr)
    ...
    local _exitcode, _stdout, _stderr = _exec_monk_cli('-datadir=data', 'getblockchaininfo')
    local _success, _output = _get_monk_cli_result(_exitcode, _stdout, _stderr)
    ...
    local _exitcode, _stdout, _stderr = _exec_monk_cli('-datadir=data', 'mnsync', 'status')
    local _success, _output = _get_monk_cli_result(_exitcode, _stdout, _stderr)
